...
def is_green(rgb, threshold=30):
    ...
    within_red_range = (
        red >= GREEN_RGB[0] - threshold and red <= GREEN_RGB[0] + threshold
    )
    within_green_range = (
        green >= GREEN_RGB[1] - threshold and green <= GREEN_RGB[1] + threshold
    )
    within_blue_range = (
        blue >= GREEN_RGB[2] - threshold and blue <= GREEN_RGB[2] + threshold
    )
...
def frame_to_text(image, START_POSITION, END_POSITION, order, which):
    ...
    for i in range(rectange.shape[0]):
        for j in range(rectange.shape[1]):
            if (
                rectange[i][j][0] > 210
                and rectange[i][j][1] > 210
                and rectange[i][j][2] > 210
            ):
                rectange[i][j] = [255, 255, 255]
            elif (
                rectange[i][j][0] < 80
                and rectange[i][j][1] < 80
                and rectange[i][j][2] < 80
            ):
                rectange[i][j] = [0, 0, 0]
            else:
    ...
    for i in range(rectange.shape[0]):
        for j in range(rectange.shape[1]):
            if (
                rectange[i][j][0] == 255
                and rectange[i][j][1] == 255
                and rectange[i][j][2] == 255
            ):
                contains_white = True
                break
    ...
    if contains_white:
        for i in range(rectange.shape[0]):
            for j in range(rectange.shape[1]):
                if (
                    rectange[i][j][0] == 0
                    and rectange[i][j][1] == 0
                    and rectange[i][j][2] == 0
                ):
                    rectange[i][j] = [255, 0, 255]
    ...
    question_text = (
        pytesseract.image_to_string(rectange, lang="ces").replace("\n", " ").rstrip()
    )

    return question_text


def perform_ocr(image, correct_answer, order):
    """
    ...
    answer_a_text = frame_to_text(
        image, START_ANSWER_A_POSITION, END_ANSWER_A_POSITION, order, "A"
    )
    answer_b_text = frame_to_text(
        image, START_ANSWER_B_POSITION, END_ANSWER_B_POSITION, order, "B"
    )
    answer_c_text = frame_to_text(
        image, START_ANSWER_C_POSITION, END_ANSWER_C_POSITION, order, "C"
    )
...
def convert_video_to_text(path, order=0):
    ...
    while success:
        for answer_index in range(len(ANSWER_POSITIONS)):
            if is_green(image[ANSWER_POSITIONS[answer_index]]):
                answer_text = perform_ocr(image, answer_index, order)
                print(answer_text)
                # skip 15 seconds
                count += 15 * fps
                vidcap.set(cv2.CAP_PROP_POS_FRAMES, count)
